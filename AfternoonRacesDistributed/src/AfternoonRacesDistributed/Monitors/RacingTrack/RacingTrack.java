...
import AfternoonRacesDistributed.Constants.Constants;
import AfternoonRacesDistributed.Entities.Broker.BrokerInterfaces.IBroker_RacingTrack;
...
public class RacingTrack implements IBroker_RacingTrack, IHorse_RacingTrack{
    ...
    /**
     * Sends INC_RACE_ITERATION message type to repository
     */
    private void incRaceIteration() {
        ...
        cc_repository.writeObject(new RepositoryMessage(RepositoryMessage.INC_RACE_ITERATION));
        response = (RepositoryMessage) cc_repository.readObject();
        cc_repository.close();
    }
    
    /**
     * Sends ADD_HORSE_RESULT message type to repository
     * @param result is the array which contains the results of the race
     */
    private void addHorseResult(int[] result) {
        RepositoryMessage response;
        while(!cc_repository.open()){
            try{
                Thread.sleep(1000);
        ...
        cc_repository.writeObject(new RepositoryMessage(RepositoryMessage.ADD_HORSE_RESULT, result));
        response = (RepositoryMessage) cc_repository.readObject();
        cc_repository.close();
    }
    ...
    public boolean hasFinishLineBeenCrossed(int horse_id) 
    {
        ...
        try {
            if(positions.get(horse_id) > length) { // crossed finish line
                ...
                addHorseResult(result); // add the result of the horse to the queue
